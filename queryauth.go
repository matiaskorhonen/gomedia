// Based on: https://github.com/goji/httpauth
//
// MIT License
//
...
//
// Modified to allow authentication via query params
//
...
func (b *basicAuth) authenticate(r *http.Request) bool {
	...
	query := r.URL.Query()
	qUser := query.Get("username")
	qPassword := query.Get("password")

	// Confirm the request is sending Basic Authentication credentials.
	auth := r.Header.Get("Authorization")
	// if !strings.HasPrefix(auth, basicScheme) || !(qUser != "" && qPassword != "") {
	// 	return false
	// }
	...
	} else if qUser != "" && qPassword != "" {
		givenUser = sha256.Sum256([]byte(qUser))
		givenPass = sha256.Sum256([]byte(qPassword))
	} else {
		return false
	}
